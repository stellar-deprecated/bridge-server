...
It can optionally be connected to a `compliance_server` if you want to carry out the compliance protocol.
...
`send_max` | optional | Maximum amount of send_asset to send
...
`path[n][asset_code]` | optional | If the path isn't specified the bridge server will find the path for you. Asset code of `n`th asset on the path (XLM when empty, but empty parameter must be sent!)
...
Check [`SubmitTransactionResponse`](./src/github.com/stellar/bridge/horizon/submit_transaction_response.go) struct.
