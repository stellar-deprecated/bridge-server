...
func (rh *RequestHandler) Payment(w http.ResponseWriter, r *http.Request) {
	...
	if request.ID != "" {
		...
		if sentTransaction == nil {
			...
			submitResponse, submitError = rh.Horizon.SubmitTransaction(sentTransaction.EnvelopeXdr)
		}
	}
	...
	if !transactionExists {
		...
		if rh.Config.Compliance != "" &&
			...
			submitResponse, submitError = rh.TransactionSubmitter.SignAndSubmitRawTransaction(paymentID, request.Source, &tx)
		} else {
			...
			submitResponse, submitError = rh.TransactionSubmitter.SubmitTransaction(paymentID, request.Source, operationBuilder, memoMutator)
		}
	}
